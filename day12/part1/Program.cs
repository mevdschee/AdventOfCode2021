...
namespace Program
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            path.Add("start");
            visits["start"] = 1;
            Console.WriteLine(Search(path, visits, vectors).Count);
        }

        private static Dictionary<List<string>, bool> Search(List<string> path, Dictionary<string, int> visits, Dictionary<(string, string), bool> vectors)
        {
            ...
            foreach (var (from, to) in vectors.Keys)
            {
                ...
                if (visits.GetValueOrDefault(to,0)>0)
                {
                    continue;
                }
                ...
                var foundPaths = Search(newPath, newVisits, vectors);
